...
class NBAProjectionModel:
    ...
    def ensemble_prediction(self, features_df, models, feature_columns, target_name='fantasy_points'):
        ...
        return ensemble_pred, pred_std, individual_predictions

def get_player_info(player_name):
    """
    Get player's common information from NBA_API based on their name
    """
    ...
    try:
        ...
        print(f"POSITION: {player_data.get('POSITION_SHORT')}")
        return {
            'id': player_id,
            'team': player_data.get('TEAM_ABBREVIATION', 'UNK'),
            'team_name': player_data.get('TEAM_NAME', 'Unknown'),
            'position': player_data.get('POSITION_SHORT', player_data.get('POSITION', 'Unknown'))
        }
...
def get_player_position(player_name):
    ...
    """
    player_positions = {}
    with open("players_positions.csv", "r", encoding="utf-8") as f:
        reader = csv.reader(f)
        for row in reader:
            name, position = row
...
def load_latest_usage_data():
    """
    Load the latest usage data from pickle file
    """
    try:
        pkl_file = 'nba_usage_rates_latest.pkl'
        if os.path.exists(pkl_file):
            # Check if file less than 24 hours old
            file_time = datetime.fromtimestamp(os.path.getmtime(pkl_file))
            if datetime.now() - file_time < timedelta(hours=24):
                usage_df = pd.read_pickle(pkl_file)
...
def update_usage_data_if_needed():
    """
    Check if usage data needs to be updated and run scraper if necessary
    """
    pkl_file = 'nba_usage_rates_latest.pkl'
    
    if not os.path.exists(pkl_file):
        print("No usage data found. Running scraper...")
        return nba_usage_rate_scraper.cache_usage_data()
    
    # Check if data is older than 24 hours
    file_time = datetime.fromtimestamp(os.path.getmtime(pkl_file))
    if datetime.now() - file_time > timedelta(hours=24):
        print("Usage data is stale. Running scraper...")
        return nba_usage_rate_scraper.cache_usage_data()
    
    return load_latest_usage_data()
    
def fetch_recent_gamelog(player_id, player_team_abbr, seasons=['2025-26']):
    """
    Fetches a player's game logs and calculates usage rate.
    """
    ...
    if not all_games.empty:
        ...
        for player in nba_players:
            if player['id'] == player_id:
                player_name = player['full_name'] # Or player['display_first_last']
                break

            if player_name:
                print(f"The player with ID {player_id} is: {player_name}")
            else:
                print(f"Player with ID {player_id} not found.")

        all_games['USAGE_RATE'] = calculate_usage_rate(player_name)
        
        all_games.sort_values(by='GAME_DATE', ascending=False, inplace=True)
    else:
        print("No games found for player.")
...
def get_player_team_id(player_id):
    """
    Fetches the team ID for a given player ID for the current season.
    """
    try:
        info = commonplayerinfo.CommonPlayerInfo(player_id=player_id)
        ...
        if not df.empty:
            ...
            return team_id if team_id != 0 else None  # 0 means not currently on a team
        else:
...
def get_team_schedule(team_id):
    ...
    Uses nba_api's ScoreboardV2 endpoint (official schedule data).
    """
    try:
        today = datetime.now().strftime("%Y-%m-%d")
        
        # Fetch today's games from the official NBA stats API
        board = scoreboardv2.ScoreboardV2(game_date=today)
        df_games = board.game_header.get_data_frame()
        
        # Check if team_id appears as either home or visitor
        game = df_games[(df_games["HOME_TEAM_ID"] == team_id) | (df_games["VISITOR_TEAM_ID"] == team_id)]
...
def get_tonights_game_context(player_name, spread, total):
    ...
    """
    # First, get player info to find their ID and team
    ...
    if game_data['HOME_TEAM_ID'] == team_id:
        is_home = True
        opponent_team_id = game_data['VISITOR_TEAM_ID']
    else:
        is_home = False
        opponent_team_id = game_data['HOME_TEAM_ID']
    ...
    # Now get the odds for this game
    # odds_data = get_game_odds(f"{player_info['team_name']} vs {opponent_team}")
...
def get_cached_defense_data():
    """
    Get defense data with caching - only scrape once per day
    """
    cache_file = 'defense_data_cache.pkl'
        ...
        if datetime.now() - file_time < timedelta(hours=24):
            try:
                with open(cache_file, 'rb') as f:
                    data = pickle.load(f)
                print("Using cached defense data")
                return data
            except:
                print("Cache corrupted, re-scraping...")
    
    # Scrape fresh data using the new Hashtag Basketball scraper
    print("Scraping fresh defense data from Hashtag Basketball...")
    from nba_defense_scraper import scrape_hashtag_basketball_table4
    data = scrape_hashtag_basketball_table4()
    
    # Cache the data
    if data:
        with open(cache_file, 'wb') as f:
            pickle.dump(data, f)
        print("Defense data cached for 24 hours")
...
def get_opponent_defense_stats(opponent_team, player_position):
    ...
    opponent_full = get_team_full_name(opponent_team)
    
    # Look for the team in the data - handle different team name formats
    team_row = None
    for idx, row in position_data.iterrows():
        team_name = str(row.get('Team', '')).upper().strip()
        
        # Check various team name formats
        if (opponent_team.upper() in team_name or 
            opponent_full.upper() in team_name or
            team_name in [opponent_team.upper(), opponent_full.upper()]):
            team_row = row
            break
    ...
    # Extract the stats we need from the new data structure
    defense_stats = {
        'opp_pts_allowed': safe_float(team_row.get('PTS', 25.0)),
        'opp_reb_allowed': safe_float(team_row.get('REB', 8.0)),
        'opp_ast_allowed': safe_float(team_row.get('AST', 6.0)),
        'opp_stl_allowed': safe_float(team_row.get('STL', 1.5)),
        'opp_blk_allowed': safe_float(team_row.get('BLK', 1.0)),
        'opp_to_forced': safe_float(team_row.get('TO', 3.0)),
        'opp_fg_pct_allowed': safe_float(team_row.get('FG%', 45.0)),
        'opp_3pm_allowed': safe_float(team_row.get('3PM', 2.5)),
    }
    
    # Calculate FD points allowed based on the new stats
    # FanDuel scoring: PTS=1, REB=1.2, AST=1.5, STL=2, BLK=2, TO=-1
    fd_pts_allowed = (
        defense_stats['opp_pts_allowed'] * 1 +
        defense_stats['opp_reb_allowed'] * 1.2 +
        defense_stats['opp_ast_allowed'] * 1.5 +
        defense_stats['opp_stl_allowed'] * 2 +
        defense_stats['opp_blk_allowed'] * 2 -
        defense_stats['opp_to_forced'] * 1
    )
    
    defense_stats['opp_fd_allowed'] = fd_pts_allowed
